/* Fetch ladder Logo URLs from Apps Script getTeamInfo and save into apps/coach-app/public/assets/team-logos/
   Usage: node scripts/fetch-squadi-logos.cjs
*/
const https = require('https');
const fs = require('fs');
const path = require('path');

const API_URL = 'https://script.google.com/macros/s/AKfycbz3DmnPOLstWmOmJs4nzDQn42XXWe0E2ujLpmfo4e4WZFkInXxUdeL8-W0SImYj9EQj/exec?api=true&action=getTeamInfo&teamID=team_1770421147222&forceRefresh=true';
...
https.get(API_URL, (res) => {
  // Follow redirects for Apps Script responses (they often 302 -> script.googleusercontent)
  if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
    const redirected = res.headers.location;
    return https.get(redirected, (r2) => {
      let body = '';
      r2.setEncoding('utf8');
      r2.on('data', d => body += d);
      r2.on('end', async () => {
    ...
    }).on('error', (err) => { console.error('API redirect fetch failed', err); process.exit(1); });
  }

  let body = '';
  res.setEncoding('utf8');
  res.on('data', d => body += d);
  res.on('end', async () => { handleApiBody(body); });
}).on('error', (err) => { console.error('API fetch failed', err); process.exit(1); });

async function handleApiBody(body) {
  let js;
