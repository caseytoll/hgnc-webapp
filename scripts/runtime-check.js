        ...
        for (const f of frames) {
          try {
            await f.evaluate((owner) => {
              ...
              try { var addGameButton = document.getElementById('show-add-game-modal'); if (addGameButton) { console.log('[runtime-check] Found add-game button, removing hidden'); addGameButton.classList.remove('hidden'); addGameButton.onclick = function() { if (typeof showGameEditor === 'function') showGameEditor('add'); }; console.log('[runtime-check] Add-game button updated'); } else { console.log('[runtime-check] Add-game button NOT found'); } } catch(e) { console.log('[runtime-check] Error updating add-game button:', e); }
            }, OWNER_EMAIL);
          } catch(e) {
