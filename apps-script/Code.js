...
function fetchOpponentDivisionStats(teamID, opponentName) {
  try {
    ...
    Logger.log('fetchOpponentDivisionStats: fetching from Squadi API for ' + opponentName);
    var apiResult = fetchSquadiFixtures(config.competitionId, config.divisionId);
...
function backfillDivisionScoutingForDivision(competitionId, divisionId, competitionKey, forceRefresh) {
  ...
  if (!sheet) {
    ...
    sheet.getRange(1, 1, 1, dfHeaders.length).setValues([dfHeaders]);
    sheet.getRange(1, 1, 1, dfHeaders.length).setFontWeight('bold');
    sheet.setFrozenRows(1);
  }
  var now = new Date().toISOString();
  ...
  (apiResult.rounds || []).forEach(function(round) {
    ...
    (round.matches || []).forEach(function(match) {
      ...
      // Always append the row (totals only — quarter fetching removed)
      result.gamesSaved++;
      sheet.appendRow(row);
    });
  });
